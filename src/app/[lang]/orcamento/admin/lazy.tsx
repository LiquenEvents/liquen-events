 ...
 *
 * Medido (CPU 4x mais lenta, 60 ms de latência): mesmo com o chunk JÁ em
 * memória, o primeiro clique numa vista mostrava o esqueleto durante 302 ms
 * (mediana de 36 cliques) e só assentava aos 361 ms. Voltar mais tarde à mesma
 * vista custava 48 ms, sem esqueleto nenhum.
 *
 ...
 * evitar piscar). Ou seja: pré-carregar o chunk sozinho NÃO tornava o clique
 * instantâneo — apenas trocava espera de rede por espera do React.
 *
 * `splitView` resolve as duas coisas ao mesmo tempo:
 ...
 * Se ainda não chegou, cai no `dynamic()` de sempre, com o esqueleto.
 *
...
interface SplitView<P> {
  ...
  /** Traz o módulo para memória. Idempotente; erros de rede não ficam colados. */
  warm: () => Promise<void>;
}
...
/* `Overview` é a única vista desenhada no servidor à chegada (AdminClient
 * arranca sempre em "overview"), e é a forma literal `dynamic(() => import(…))`
 * que o Next reconhece para emitir o `<link rel="preload">` do chunk no HTML.
 ...
 * em AdminClient) mas nunca o CÓDIGO. Aqui aquecemos os módulos das vistas,
 * um de cada vez, pela ordem em que aparecem na barra lateral.
 *
 * Regras de segurança: nunca antes do primeiro desenho (espera-se pelo `load`),
 * nunca em ligações medidas ou muito lentas (Save-Data, 2g) e nunca em
 * desenvolvimento — em dev, importar todas as vistas obrigaria o servidor a
 * compilá-las todas, tornando o trabalho do dia-a-dia MAIS lento, não mais
 * rápido. Tudo junto são ~220 KB (comprimidos ~90 KB), menos do que o runtime
 * do React, e ficam em cache imutável durante um ano.
 */
 ...
 * primeiro, depois as de "Mais"), para que o módulo mais provável esteja
 * pronto primeiro. No fim ficam as vistas que estão fora do menu
 * (`inventario`, `modelos-email`) — só se lá chega por link directo.
 */
...
// Arranque automático no browser. Em dev fica desligado de propósito (ver nota
// acima); os testes chamam `warmViewChunks` directamente com um agendador falso.
